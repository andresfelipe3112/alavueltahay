...
const PickerImage = ({ isUpdatePanorama = false, ...props }) => {
  const { onClose, handleImg } = props;

  const [filePath, setFilePath] = useState(props.imagePanorama || null);
  ...
  const savePhoto = async (method) => {
    try {
      if (method === 1) {
        ...
        if (filePath && _jwt && filePath?.uri !== props.imagePanorama?.uri) {
          const res = await dataPanorama.updateImagePanorama(
            props.imagePanorama,
            filePath,
            _jwt
          );
  ...
  return (
    <View style={styles.view}>
      ...
      {(!loading &&
        filePath?.uri &&
        filePath?.uri !== props.imagePanorama?.uri && (
          <ButtonComponent
            text="Guardar"
            color={GlobalVars.orange}
            textColor={GlobalVars.white}
