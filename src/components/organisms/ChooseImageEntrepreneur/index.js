...
const PickerImage = ({ isEntrepreneurUpdate = false, ...props }) => {
  const { onClose, handleImg, onCloseAllPhoto } = props;
    ...
    try {
      if (method === 2) {
        ...
        if (filePath && _jwt && entrepreneur) {
          const res = await updateDataEntrepreneur.uploadShopNewPhoto(
            props.imageShop,
            entrepreneur,
            filePath,
            _jwt
